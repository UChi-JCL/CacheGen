...
if __name__ == "__main__":
    ...
    #     gt_path = f"{args.gt_dir}/gt_{i}.txt"
    ...
    with open(gt_json_file, 'r') as json_file:
        gt_json_list = list(json_file)
    for i in range(args.start, args.end):
        gt_result = gt_json_list[i].split(',')[0].replace('Label: ', '')
        with open(f"{args.results_dir}/{args.result_str}_{i}.txt", "r") as f:
            cachegen_result = f.read()
        correct += chatgpt_auto_eval(gt_result, cachegen_result)
    print(f"accuracy: {correct/ ((args.end - args.start)) }")