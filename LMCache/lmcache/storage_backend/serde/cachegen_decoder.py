...
class CacheGenDeserializer(Deserializer):
    def __init__(self, config: LMCacheEngineConfig, metadata: LMCacheEngineMetadata, cdf: torch.Tensor):
    ...
    def from_bytes(self, bs: bytes) -> torch.Tensor:
        ...
        key, value = decode_function_gpu(
                ...
                layers_in_key,
                ntokens,
                self.get_output_buffer(self.cdf.shape[0] // 2, self.cdf.shape[1], ntokens)
            )
