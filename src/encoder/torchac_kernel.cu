        ...
        const torch::Tensor& sym)
{
    ...
    // const auto cdf_ptr = get_cdf_ptr_ours(cdf);
...
PYBIND11_MODULE(mytorchac_cuda, m) {
    ...
    m.def("encode_cdf", &encode_cdf, "encode cdf (original cpu version)");
    m.def("decode_cdf", &decode_cdf, "decode cdf (original cpu version)");
